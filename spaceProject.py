...
def find_stars_in_image(path):
    ...
    # i = 1;
    # for star in stars:
    #     print("Star {} at (x={}, y={}) with radius={} and brightness={}".format(i, star[0], star[1], star[2], star[3]))
    #     i += 1
    # return stars
...
def find_common_stars(image_path1, image_path2):
    ...
    for i in range(min(len(stars1), len(stars2))):
        ...
        if abs(dists1[idx1]-dists2[idx2]) < 200:  # Set the threshold for RANSAC
            inliers.append((stars1[i], stars2[i])) #Really is a overlapping star so we will add it to the list of common stars
